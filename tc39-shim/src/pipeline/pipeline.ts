...
type PipelineOutputType<OPS extends [Func, ...any[]][], InitialValue> = OPS extends []
  ? InitialValue
  : OPS extends [[infer F, ...infer Args], ...infer RestOPS]
    ? RestOPS extends [Func, ...any[]][]
      ? PipelineOutputType<RestOPS, Func.Return<F>>
      : Func.Return<F>
    : never;

// Recursive type to calculate the initial input type required by the pipeline
type PipelineInputType<OPS extends [Func, ...any[]][]> = OPS extends []
  ? any // Or unknown, depending on desired strictness
  : OPS extends [[infer F extends Func, ...infer _Args], ...infer _RestOPS]
    ? FirstArgType<F>
    : never;

// The main Pipeline type
export type Pipeline<OPS extends [Func, ...any[]][]> = (initialValue: PipelineInputType<OPS>) => PipelineOutputType<OPS, PipelineInputType<OPS>>;
