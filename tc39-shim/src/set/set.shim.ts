...
export const set_intersection = safeSetFn<"intersection">(
    <T, U>(self: Set<T>, other: ReadonlySetLike<U>): Set<T & U> => {
        CheckSetRecord(other);
 ...
 */
export const set_symmetric_difference = safeSetFn<"symmetricDifference">(
    <T, U>(self: Set<T>, other: ReadonlySetLike<U>): Set<T | U> => {
        CheckSetRecord(other);
        const result = new Set<T | U>(self);
        for (const elem of iterator_iterable(other.keys())) {
...
export const set_is_subset_of = safeSetFn<"isSubsetOf">(
    <T>(self: Set<T>, other: ReadonlySetLike<unknown>): boolean => {
        CheckSetRecord(other);
        for (const elem of self) {
...
export const set_is_superset_of = safeSetFn<"isSupersetOf">(
    <T>(self: Set<T>, other: ReadonlySetLike<unknown>): boolean => {
        CheckSetRecord(other);
        for (const elem of iterator_iterable(other.keys())) {
...
export const set_is_disjoint_from = safeSetFn<"isDisjointFrom">(
    <T>(self: Set<T>, other: ReadonlySetLike<T>): boolean => {
        CheckSetRecord(other);
        if (self.size <= other.size) {
            for (const elem of self) {
                if (other.has(elem)) return false;
