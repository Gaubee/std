 ...
 */
const decimal_self_round__ = (x: DecimalRaw, sd: number, rm: number = defaultDecimalRoundingMode, more = false) => {
  const xc = x.c;

  if (rm !== DecimalRoundingMode.ROUND_DOWN && rm !== DecimalRoundingMode.ROUND_HALF_UP && rm !== DecimalRoundingMode.ROUND_HALF_EVEN && rm !== DecimalRoundingMode.ROUND_UP) {
 ...
 */
const stringify = (x: DecimalRaw, doExponential: boolean, isNonzero: boolean) => {
  let e = x.e;
  let s = x.c.join("");
  const n = s.length;
 ...
 */
export const decimal_div = (x: DecimalRaw, y: DecimalAble) => decimal_div__(x, y, defaultDecimalPlaces, defaultDecimalRoundingMode);
...
export const decimal_pow = (x: DecimalRaw, n: number) => {
  ...
  if (n !== ~~n || n < DecimalExponent_MIN_POWER || n > DecimalExponent_MAX_POWER) {
...
export const decimal_round = (x: DecimalRaw, dp?: number, rm?: DecimalRoundingMode) => {
  ...
  else if (dp !== ~~dp || dp < DecimalPlaces_MIN_DP || dp > DecimalPlaces_MAX_DP) {
    throw Error(INVALID_DP);
  }
...
export const decimal_sqrt = (x: DecimalRaw) => {
  ...
  if (s === 0 || s === 1 / 0) {
    ...
    //@ts-ignore
    e = (((e + 1) / 2) | 0) - (e < 0 || e & 1);
    r = decimal((s === 1 / 0 ? "5e" : (s = s.toExponential()).slice(0, s.indexOf("e") + 1)) + e);
  } else {
    r = decimal(`${s}`);
  }
 ...
 */
export const decimal_toJSON = (x: DecimalRaw) => stringify(x, x.e <= defaultNegativeExponent || x.e >= defaultPositiveExponent, !!x.c[0]);
 ...
 */
export const decimal_valueOf = (x: DecimalRaw) => stringify(x, x.e <= defaultNegativeExponent || x.e >= defaultPositiveExponent, true);
