import {decimal, decimal_abs, type DecimalAble, setDefaultDecimalPlaces, setDefaultDecimalRoundingMode} from "../decimal.ts";
import {areEqual, endCheck, isNegativeZero, isPositiveZero} from "./test-helper.ts";
Deno.test("abs", function (tt) {
  function t(expected: number | string, value: DecimalAble) {
    areEqual(String(expected), decimal_abs(decimal(String(value))).toString());
  }
  ...
  t(99, 99);
  t(1, decimal(-1));
  t(0.001, decimal(0.001));
  t(0.001, decimal("-0.001"));
  t(0, 0);
  t(0, -0);

  const minusZero = 1 / (-1 / 0);

  t(0, 0);
  t(0, -0);
  t(0, minusZero);

  isNegativeZero(decimal("-0"));
  isNegativeZero(decimal(minusZero));
  isPositiveZero(decimal_abs(decimal(-0)));
  isPositiveZero(decimal_abs(decimal("-0")));
  isPositiveZero(decimal_abs(decimal(minusZero)));
