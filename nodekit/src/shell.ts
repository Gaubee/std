...
export const $$: (options: CreateShellOptions) => Shell = (options: CreateShellOptions): Shell => {
  ...
  const sh = buildSh({});
  const ext = {
    async spawn(command: string, args: string[] | string, options?: CommandOptions): Promise<void> {
      let safe_cmd = command;
      let safe_args: string[] = [];
      const safe_options: child_process.CommonSpawnOptions = obj_omit(options ?? {}, "stdin", "stdout", "stderr");
      ...
      console.info(colors.blue("⫸"), colors.magenta([command, ...safe_args].join(" ")));
