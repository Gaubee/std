...
export const GF = (() => {
  try {
    return Function(str_reverse("rotcurtsnoc.)}{)(*noitcnuf( nruter"))();
  } catch {
...
export const AGF = (() => {
  try {
    return Function(str_reverse("rotcurtsnoc.)}{)(*noitcnuf cnysa( nruter"))();
  } catch {
    return async function* () {}.constructor;
  }
})() as AsyncGeneratorFunction;

type AGValue<T> = T extends AsyncIterableIterator<infer V, unknown, unknown> ? V : unknown;
type AGReturn<T> = T extends AsyncIterableIterator<unknown, infer R, unknown> ? R : unknown;
type AGNext<T> = T extends AsyncIterableIterator<unknown, unknown, infer N> ? N : unknown;
 ...
 * ```
 */
export const ag_done = async <T extends AsyncIterableIterator<any>>(ag: T, each: AsyncGeneratorDoneOptions<T> = {}): Promise<AGReturn<T>> => {
  let next: any;
  while (true) {
...
export const ag_map = async <T extends AsyncIterableIterator<any>, R>(ag: T, each: AsyncGeneratorMapOptions<T, R>): Promise<R[]> => {
  const result: R[] = [];
  await ag_done(ag, {
    next: async (value) => {
...
export const ag_map_reduce = async <T extends AsyncIterableIterator<any>, M, R>(ag: T, each: AsyncGeneratorMapReduceOptions<T, M, R>): Promise<R> => {
 ...
 * ```
 */
export const ag_then = <T extends AsyncIterableIterator<any>, ARGS extends Parameters<Promise<AGReturn<T>>["then"]> = Parameters<Promise<AGReturn<T>>["then"]>>(
  ag: T,
  ...args: ARGS
): Promise<ReturnType<FilterNotNull<ARGS>[number]>> => {
