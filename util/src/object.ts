 ...
 */
export const obj_pick = <T extends object, KS extends (keyof T)[] = (keyof T)[]>(obj: T, ...props: KS): Pick<T, KS[number]> => {
 ...
 * ```
 */
export const obj_pick_as = <T extends object, const A extends Alias<T>>(obj: T, alias: A): PickAs<T, A> => {
 ...
 * ```
 */
export const obj_omit = <T extends object, KS extends (keyof T)[] = (keyof T)[]>(obj: T, ...props: KS): Omit<T, KS[number]> => {
 ...
 */
export const obj_lazy_builder = <T extends object>(obj: Partial<T>, get: <K extends keyof T>(target: Partial<T>, prop: K) => T[K], target = obj): T => {
 ...
 * 注意，不会遍历原型链
 */
export const obj_all_descriptors = <T extends object>(obj: T): Array<[prop: keyof T & (string | symbol), desp: PropertyDescriptor]> => {
 ...
 */
const _delegate_by = <D extends object, T extends D>(target: T, proto: D, getBy: (target: T) => D, recursive: boolean, omit: Set<PropertyKey>) => {
...
export const obj_assign_props: <A extends object, B extends object>(
  ...
  b: B & ThisType<A>,
) => A & B = <A extends object, B extends object>(a: A, b: B & ThisType<A>) => {
 ...
 */
/*@__NO_SIDE_EFFECTS__*/
export const obj_assign_safe_props: <A extends object>(a: A) => <B extends object>(b: B & ThisType<A & B>) => A & B = <A extends object>(a: A) => {
