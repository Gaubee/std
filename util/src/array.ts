 ...
 * 如果是稀疏数组，会跳过稀疏部分，取第一个
 */
export const sparse_arr_get_first_or_null = <T>(arr: ArrayLike<T>): T | undefined => sparse_arr_get_first_or_default(arr, () => void 0);
/**
 * 取数字第一个元素
 * 如果是稀疏数组，会跳过稀疏部分，取第一个
 * 否则取默认返回值
 */
export const sparse_arr_get_first_or_default = <T, R>(arr: ArrayLike<T>, defaultValue: () => R): T | R => {
  if (arr.length <= 0) {
    return defaultValue();
  }
  if (0 in arr) {
 ...
 * 注意，如果不存在，会抛出异常
 */
export const arr_get_first = <T>(arr: ArrayLike<T>): T =>
 ...
 * 取数字第一个元素（下标 0）
 */
export const arr_get_first_or_null = <T>(arr: ArrayLike<T>): T | undefined => arr_get_first_or_default(arr, () => void 0);
/**
 * 取数字第一个元素（下标 0）
 *
 * 否则取默认返回值
 */
export const arr_get_first_or_default = <T, R>(arr: ArrayLike<T>, defaultValue: () => R): T | R => {
  if (arr.length <= 0) {
    return defaultValue();
  }
  return arr[0];
 ...
 * 如果是稀疏数组，会跳过稀疏部分
 */
export const sparse_arr_get_last_or_null = <T>(arr: ArrayLike<T>): T | undefined => sparse_arr_get_last_or_default(arr, () => void 0);
 ...
 * 否则取默认返回值
 */
export const sparse_arr_get_last_or_default = <T, R>(arr: ArrayLike<T>, defaultValue: () => R): T | R => {
 ...
 * 注意，如果不存在，会抛出异常
 */
export const arr_get_last = <T>(arr: ArrayLike<T>): T =>
 ...
 * 取数字最后一个元素（下标 length-1）
 */
export const arr_get_last_or_null = <T>(arr: ArrayLike<T>): T | undefined => arr_get_last_or_default(arr, () => void 0);
 ...
 * 否则取默认返回值
 */
export const arr_get_last_or_default = <T, R>(arr: ArrayLike<T>, defaultValue: () => R): T | R => {
  const at = arr.length - 1;
  if (at < 0) {
    return defaultValue();
  }
