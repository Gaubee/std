 ...
 * 可以是负数
 */
export const date_add_duration = (date: Date, duration: Partial<Duration>): Date => {
  const {year = NaN, month = NaN, day = NaN, minutes = NaN, seconds = NaN, milliseconds = NaN} = duration;
 ...
 * 对一个 date 对象进行写入
 */
export const date_set_duration = (date: Date, duration: Partial<Duration>): Date => {
  const {year = NaN, month = NaN, day = NaN, minutes = NaN, seconds = NaN, milliseconds = NaN} = duration;
