...
export namespace Timmer {
  ...
  export type FromType<T extends number | Timmer<any>> = T extends number ? void : T extends Timmer<infer R> ? R : never;
}
...
export const timmers = {
  ...
  eventTarget: <T extends Event>(target: Pick<EventTarget, "addEventListener" | "removeEventListener">, eventType: string, filter?: (event: T) => boolean | void): Timmer<T> => {
    return (resolve) => {
      let cb: (evt: T) => void;
 ...
 */
export const delay = <T extends number | Timmer<any>>(ms: T, options?: {signal?: AbortSignal | null; disposer?: PureEvent<void>}): delay.Delayer<Timmer.FromType<T>> => {
 ...
 * @returns
 */
export const promise_once_then = <P extends PromiseLike<any>, T = Awaited<P>>(promise: P, task: PromiseOnceThenTask<T>): (() => void) => {
...
export const promise_once_then_cancel = <P extends PromiseLike<any>, T = Awaited<P>>(promise: P, task: PromiseOnceThenTask<T>): void => {
...
export const promise_safe_race = <V extends Iterable<any>, T = Awaited<IterableItem<V, unknown>>>(values: V, options: PromiseRaceOptions<T> = {}): PromiseMaybe<T> => {
