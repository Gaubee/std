...
export const abort_signal_promisify = (input: AbortSignal | /*AbortController*/ {signal: AbortSignal}): Promise<never> => {
...
export const abort_signal_race = <T>(signal: AbortSignal, fn_or_promise: PromiseLike<T> | (() => PromiseLike<T>)): PromiseMaybe<T> => {
