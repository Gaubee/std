    ...
    return () => {
        ...
        fn(
            Object.assign(abort.signal, {
                race: (...args: Slice1<Parameters<typeof abort_signal_race>>) =>
                    abort_signal_race(abort.signal, ...args),
...
export const useAsyncLayoutEffect = (
    fn: (signal: AbortSignalWithRace) => Promise<void>,
    deps?: DependencyList,
): void => {
    useLayoutEffect(asyncEffectCallback(fn), deps);
};

export const useAsyncInsertionEffect = (
    fn: (signal: AbortSignalWithRace) => Promise<void>,
    deps?: DependencyList,
): void => {
